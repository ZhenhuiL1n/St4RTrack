...
# This script is used to train the model with TTA using METRIC DEPTH supervision.
# Uses calibrated depth (GT foreground + scaled MoGe background) with scale-aware loss.
...
CUDA_VISIBLE_DEVICES=0,5,6 torchrun --nproc_per_node=3 --master_port=29637 train.py \
...
--train_dataset "300 @ CustomDUSt3R(S=12, resolution=[(512, 512)], \
