...
# TTA (Test-Time Adaptation) for a SINGLE DNA sequence with ALL 48 VIEWS
# Uses CALIBRATED MoGe depth from calibrated_depth folder
# Uses DNAMultiSeqDataset which handles multi-view with per-view calibrated depth
...
# Only include the target sequence, exclude all others
# This creates a "TTA" effect with DNAMultiSeqDataset

CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=29641 train.py \
...
--train_dataset "300 @ DNASingleSeqDataset(dataset_location='${DATA_DIR}/${SEQ_NAME}', \
moge_depth_dir='${MOGE_DIR}', \
...
cotracker=False, align3d_weight=0, metric_depth_weight=2.0)" \
--output_dir "./train_TTA_results/${SEQ_NAME}_48views_calibrated_moge"
