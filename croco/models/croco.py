...
class CroCoNet(nn.Module):
    ...
    def __init__(self,
        ...
        if pos_embed=='cosine':
            ...
            self.rope_enc = RoPE2D(freq=freq)
            self.rope_dec = RoPE2D(freq=freq)
        else:
        ...
        self.enc_blocks = nn.ModuleList([
            Block(enc_embed_dim, enc_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer, rope=self.rope_enc)
